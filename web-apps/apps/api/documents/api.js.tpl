...
void 0!==this.editorInfo.type&&delete this.editorInfo.type;if(this.editorInfo.theme&&this.onThemeChanged)this.onThemeChanged(this.editorInfo.theme);break;case "onTheme":this.editorInfo.theme=b.theme;if(this.editorInfo.theme&&this.onThemeChanged)this.onThemeChanged(this.editorInfo.theme);break;case "onWindowEvent":if("private_window_method"===b.eventName){var c=b.windowID,d=this;this.executeMethod(b.eventData.name,b.eventData.params,function(f){d._windows&&d._windows[c]&&d._windows[c].dispatchEvent("on_private_window_method",
f)})}else"private_window_command"===b.eventName?(c=b.windowID,d=this,this.callCommand(b.eventData.code,function(f){d._windows&&d._windows[c]&&d._windows[c].dispatchEvent("on_private_window_command",f)},!1===b.eventData.isCalc?!0:void 0)):this._windows[b.windowID]._oncommand(b.eventName,b.eventData);break;case "onWindowButton":if(this._windows&&b.windowID&&this._windows[b.windowID])if(-1===b.button)this._windows[b.windowID].close();else if(this._windows[b.windowID].onButtonClick)this._windows[b.windowID].onButtonClick(b.button)}}};
e.prototype.sendMessage=function(a){var b={frameEditorId:"iframeEditor",type:"onExternalPluginMessage",subType:"connector"};b.data=a;b.data.guid=this.guid;a=this.frame;"string"===typeof a&&(a=document.getElementById(this.frame));a&&a.contentWindow.postMessage(JSON.stringify(b),"*")};e.prototype.connect=function(){this.isConnected?console.log("This connector is already connected"):(window.addEventListener?window.addEventListener("message",this.onMessageBound,!1):window.attachEvent&&window.attachEvent("onmessage",
this.onMessageBound),this.isConnected=!0,this.sendMessage({type:"register"}))};e.prototype.disconnect=function(){this.isConnected?(window.removeEventListener?window.removeEventListener("message",this.onMessageBound,!1):window.detachEvent&&window.detachEvent("onmessage",this.onMessageBound),this.isConnected=!1,this.sendMessage({type:"unregister"}),this.sendMessage({type:"unregister"})):console.log("This connector is already disconnected")};e.prototype.callCommand=function(a,b,c){this.isConnected?(this.callbacks.push(b),
a={type:"command",recalculate:!0===c?!1:!0,data:"string"===typeof a?a:"var Asc = {}; Asc.scope = "+JSON.stringify(window.Asc.scope||{})+"; var scope = Asc.scope; ("+a.toString()+")();"},1!==this.callbacks.length?this.tasks.push(a):this.sendMessage(a)):console.log("Connector is not connected with editor")};e.prototype.executeMethod=function(a,b,c){this.isConnected?(this.callbacks.push(c),a={type:"method",methodName:a,data:b},1!==this.callbacks.length?this.tasks.push(a):this.sendMessage(a)):console.log("Connector is not connected with editor")};
e.prototype.attachEvent=function(a,b){this.isConnected?(this.events[a]=b,this.sendMessage({type:"attachEvent",name:a})):console.log("Connector is not connected with editor")};e.prototype.detachEvent=function(a){this.events[a]&&(delete this.events[a],this.isConnected?this.sendMessage({type:"detachEvent",name:a}):console.log("Connector is not connected with editor"))};e.prototype.sendEvent=function(a,b){return this.executeMethod("SendEvent",[a,b],function(){})};e.prototype._correctCustomMenuItems=function(a,
b){var c={guid:this.guid};b.tabs?c.tabs=[]:c.items=[];let d=function(p,h){let l={id:void 0!==h.id?h.id:n()};for(prop in h)switch(prop){case "id":case "items":case "onClick":break;case "url":case "icons":l[prop]=(new URL(h[prop],location.href)).href;break;default:l[prop]=h[prop]}if(h.items){l.items=[];for(var q=0,r=h.items.length;q<r;q++)l.items.push(d(p,h.items[q]))}else h.onClick&&(p[a][l.id]=!0,p.attachEvent(l.id,h.onClick));return l};if(b.tabs){c.tabs=[];for(var f=0,m=b.tabs.length;f<m;f++)c.tabs.push(d(this,
b.tabs[f]))}else for(c.items=[],f=0,m=b.length;f<m;f++)c.items.push(d(this,b[f]));return c};e.prototype._addCustomMenuEvent=function(a){let b="";"contextMenuEvents"==a?b="onContextMenuClick":"toolbarMenuEvents"==a&&(b="onToolbarMenuClick");this.events[b]||this.attachEvent(b,function(d){var f=void 0,m=d.indexOf("_oo_sep_");-1!==m&&(f=d.substring(m+8),d=d.substring(0,m));this.events[d]&&this.events[d].call(this,f)}.bind(this));if(this[a])for(var c in this[a])this[a].hasOwnProperty(c)&&this.detachEvent(c);
this[a]={}};e.prototype.addContextMenuItem=function(a){this._addCustomMenuEvent("contextMenuEvents");this.executeMethod("AddContextMenuItem",[this._correctCustomMenuItems("contextMenuEvents",a)])};e.prototype.updateContextMenuItem=function(a){this.executeMethod("UpdateContextMenuItem",[this._correctCustomMenuItems("contextMenuEvents",a)])};e.prototype.addToolbarMenuItem=function(a){this._addCustomMenuEvent("toolbarMenuEvents");this.executeMethod("AddToolbarMenuItem",[this._correctCustomMenuItems("toolbarMenuEvents",
a)])};g.prototype._register=function(){this.connector._windows||(this.connector._windows={});this.connector._windows[this.id]=this};g.prototype._unregister=function(){this.connector._windows&&this.connector._windows[this.id]&&delete this.connector._windows[this.id]};g.prototype.show=function(a){var b=(new URL(a.url,location.href)).href;b=-1===b.indexOf(".html?")?b+"?windowID=":b+"&windowID=";b+=this.id+"&guid="+encodeURIComponent(this.connector.guid);a.url=b;a.icons&&(a.icons=(new URL(a.icons,location.href)).href);
this._register();this.connector.executeMethod("ShowWindow",[this.id,a])};g.prototype.activate=function(){this.connector.executeMethod("ActivateWindow",[this.id])};g.prototype.close=function(){this._oncommand("onClose");this.connector.executeMethod("CloseWindow",[this.id]);this._unregister()};g.prototype.dispatchEvent=function(a,b){this.connector.executeMethod("SendToWindow",[this.id,a,"object"===typeof b?JSON.stringify(b):b])};g.prototype.attachEvent=function(a,b){this._events[a]=b};g.prototype.detachEvent=
function(a){this._events&&this._events[a]&&delete this._events[a]};g.prototype._oncommand=function(a,b){this._events&&this._events[a]&&this._events[a].call(this,b)};e.prototype.createWindow=function(){return new g(this)};k.Asc=k.Asc?k.Asc:{};k.Asc.EditorConnector=e;k=!1;try{new Function("async function test() {}"),k=!0}catch(a){k=!1}k&&(eval("Asc.EditorConnector.prototype.callCommandAsync = function(func) { var t = this; return new Promise(function(resolve) { t.callCommand(func, function(retValue) { resolve(retValue); }) }); };"),
eval("Asc.EditorConnector.prototype.callMethodAsync = function(name, args) { var t = this; return new Promise(function(resolve) { t.executeMethod(name, args || [], function(retValue) { resolve(retValue); }) }); };"))})(window);

        function _createConnector(settings) { return new Asc.EditorConnector({frame : iframe, autoconnect : (settings ? settings.autoconnect : true)}); }
